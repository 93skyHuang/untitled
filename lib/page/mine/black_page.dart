...
import 'package:untitled/network/bean/black_info.dart';
...
class _BlackPageState extends State<BlackPage>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("黑名单", style: TextStyle(fontSize: 17, color: Colors.black)),
        backgroundColor: Color(0xFFF5F5F5),
        centerTitle: true,
      ),
          ...
          child: SmartRefresher(
            ...
            child: ListView.builder(
              itemBuilder: (context, index) {
                return ItemFollow(
                  name: '${blacks[index].cname}',
                  img: blacks[index].headImgUrl ?? "",
                  info: ' ',
                  onPressed: () {
                    // Get.to(() => UserHomePage(uid: blacks[index].uid ?? 0));
                  },
                  onMorePressed: () {
                    showBottomOpen(context, blacks[index].uid ?? 0);
                  },
                );
              },
              itemCount: blacks.length,
            ),
          )),
    );
  }
  ...
  void getList({bool isLoad = false}) {
    getPullBlackList(pageNo).then((value) => {
          if (value.isOk())
            if (isLoad)
              {
